...
''', encoding='utf-8')
    (rep/'FINAL_CLOSURE_STATUS.md').write_text('''# Final closure status

## Gate: PAPER_READY_WITH_MINOR_FIXES

The final paper package is synchronized to frozen source artifacts. No experiment, model, threshold, event registry, or numeric result was changed in this build. Main figures are limited to five bilingual figures; legacy H4 contribution/lift metrics are excluded from formal tables, captions, narratives, and figures (except the explicit audit record).

Package-level checks passed: Python syntax compilation, zero figure-build warnings, bilingual PNG/PDF/SVG parity, 300-dpi PNG metadata, manifest completeness, and SHA256 verification. The full repository pytest command was not used as a scientific gate here because collection did not complete within the bounded check; this packaging result therefore carries a minor engineering QA caveat.
''', encoding='utf-8')
...
def manifest() -> None:
    ...
    rows.extend([
      ...
      ['Figure 3','RQ2','MAIN','frozen IP master; scripts/audit_and_render_figure3.py; TABLE_F07/TABLE_F08','IP; complete ROC and PR curves','Power and Normal discrimination profiles are close; corrected PR axes and full curve','No equivalence test; AP is sensitive to prevalence; class imbalance is extreme','Corrected calculation/display audit with full PR curve and inset'],
...
def main() -> None:
    ...
    subprocess.check_call([sys.executable, str(ROOT/'scripts/audit_and_render_figure3.py'), '--out', str(OUT)])
    ...
    shutil.copy2(ROOT/'scripts/audit_and_render_figure3.py', OUT/'source_snapshot/audit_and_render_figure3.py')
    files=[]

...
def render_current(root: Path) -> Path:
    ...
    # Figure 2: frozen marker geometry, display-only title and a legend matching
    # the actual V3 SVG objects (green circles = power; red crosses = war).
    audit=[]
    power_color="#2ca02c"; war_color="#d62728"
    for lang in ("zh","en"):
        ...
        overlay=(f'<g id="current_manuscript_overlay"><rect x="0" y="0" width="929.6" height="32" fill="white"/><text x="300" y="17" text-anchor="middle" font-size="11" font-family="WenQuanYi Zen Hei, DejaVu Sans, sans-serif">{title}</text><rect x="600" y="4" width="310" height="23" fill="white" fill-opacity=".97" stroke="#bbbbbb" stroke-width=".4"/><circle cx="615" cy="15" r="3.5" fill="{power_color}"/><text x="625" y="18" font-size="8" font-family="WenQuanYi Zen Hei, DejaVu Sans, sans-serif">{ptxt}</text><path d="M 755 11 L 763 19 M 763 11 L 755 19" stroke="{war_color}" stroke-width="1.5"/><text x="770" y="18" font-size="8" font-family="WenQuanYi Zen Hei, DejaVu Sans, sans-serif">{wtxt}</text></g>')
    ...
    for row in audit:
        row.update({"power_actual_marker":"circle","power_actual_color":power_color,"power_legend_marker":"circle","power_legend_color":power_color,"war_actual_marker":"cross","war_actual_color":war_color,"war_legend_marker":"cross","war_legend_color":war_color,"labels_match":"PASS"})
    pd.DataFrame(audit).to_csv(out/"qa/FIGURE2_GEOMETRY_AUDIT.csv",index=False)
    (out/"qa/FIGURE2_COHORT_AUDIT.md").write_text("# Figure 2 event-registry cohort audit\n\nThe source is the frozen V3 event-registry timeline. The approved frozen artifacts do not provide an unambiguous event-ID mapping from every registry row to the exact subset consumed by the Figure 3–7 IP analysis. **NOT RECOVERABLE**: the figure is therefore retained as temporal coverage, with no inferred highlighted analysis subset. The captions explicitly state that registry coverage is not equivalent to inclusion in the downstream IP analysis; availability, opportunity, and event-matching conditions further constrain the analysis cohort.\n",encoding="utf-8")
    ...
    (out/"README.md").write_text("# paper_current_final_v3_fixed\n\n投稿前语义一致性修正版。仅修正图表显示、图注、表格尺度表达和 QA；不重跑实验、不修改事件集、指标、阈值、标签、模型或冻结主表。Figure 2 是事件登记时间覆盖，不能被解读为全部事件均进入 IP 主分析。Table 2 将 GEE β（log-odds）与 OR 及其 OR CI 分列；不可估计的 Period × ITDK interaction 移至 analysis-status audit。\n",encoding="utf-8")

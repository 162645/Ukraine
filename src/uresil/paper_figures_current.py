...
def render_f04(run,out,lang):
    p=_paths(run); c=_read(p["s35"]/"tables/episode_count_comparison_by_oblast.csv"); c=c[c.panel.isin(["primary","augmented"])].copy(); c["n"] = c.audit_episode_n; c.to_csv(out/"figure_data/F04_episode_counts.csv",index=False); states=state_order(c.geo_name); fig,(a,b)=plt.subplots(1,2,figsize=(7,3.8),gridspec_kw={"width_ratios":[1.25,1]},constrained_layout=True); piv=c.pivot(index="geo_name",columns="panel",values="n").reindex(states).fillna(0); y=np.arange(len(states)); a.hlines(y,piv.get("primary",0),piv.get("augmented",0),color="#777",lw=1); a.scatter(piv.get("primary",0),y,color=COLORS["primary"],s=16,label=PLOT_LABELS[lang]["primary"]); a.scatter(piv.get("augmented",0),y,color=COLORS["augmented"],s=16,label=PLOT_LABELS[lang]["augmented"]); a.axvline(3,color="#333",ls="--",lw=.7); a.set_yticks(y); a.set_yticklabels([zh_name(s,lang) for s in states]); a.set_xlabel("Revised independent episode count"); a.legend(frameon=False); mat=c.assign(month=c.event_date.astype(str).str[:7] if "event_date" in c else ""); events=_read(p["s3"]/"tables/calibration_events.csv"); events["month"]=events.event_date.astype(str).str[:7]; events["episode_v2"]=events.episode_id_v2_augmented.fillna(events.episode_id_v2_main); m=events.groupby(["geo_name","month"])["episode_v2"].nunique().unstack(fill_value=0); m=m.reindex(states); b.imshow(m.values,aspect="auto",interpolation="none",cmap="Purples",vmin=0); b.set_yticks(y); b.set_yticklabels([zh_name(s,lang) for s in states]); b.set_xticks(range(len(m.columns))); b.set_xticklabels(m.columns,rotation=45,ha="right"); b.set_xlabel("Month"); b.set_title("Revised episode coverage",loc="left"); save_figure(fig,out/f"main/{lang}/F04_calibration_evidence_coverage",lang,{"figure":"F04","source":"stage03 and stage03.5 revised registry","episode_rule":"episode_id_v2; at least one clean 2h cycle separates episodes","reference":"x=3 support line","old_registry":"not used for main counts"})
